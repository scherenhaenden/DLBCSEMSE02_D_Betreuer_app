...
## Overview
The `[SetUp]` method in `AuthControllerTests` is the cornerstone of the integration testing framework. It establishes a fully isolated, production-like test environment for each test method. This document provides a detailed breakdown of each operation within the SetUp method, explaining its purpose, implementation, and necessity.
